...
function showInstructions(page,object){
        ...
        for(let i=0;i<object.textInstructions[page].length;i++){
          textInstructions = textInstructions+ '<p>'+object.textInstructions[page][i]+'</p>'
          }

        $('#Stage').html('<div class="col">'+textInstructions+'</div>');


      // When to display back button - only if there is a page to go back to
        if (page == 0){
          document.getElementById("bBack").className = "btn btn-default mt-2 mb-2 rounded  invisible myBtn";}

...
$INIT_REW	= stripslashes(htmlspecialchars($_POST['initRew']));
...
$stmt = $db->prepare("INSERT INTO TOE_V2Sliders VALUE(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?, NOW())");
$stmt->bind_param("sssidiididiiiiiddiidddd",
    $PARTID,$EXPID,$TNAME,$TRIAL,
    $RTS,
    $SYMBOL,$SYMBOLID,$SYM_PROB,$SYM_REW,$SYM_EV,
    $RESP_KEY,$INIT_PROB,$INIT_REW,
